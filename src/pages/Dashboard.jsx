...
const Dashboard = () => {
  ...
  const onClick = (role) => {
    ...
    if (personnum) {
      ...
      else if (personnum == role && personnum == 2 && role == 2 && accounts[0]) {

        window.location.replace(`/hospitalProfile?account=${accounts[0]}`);

      }
      else if (personnum == role && personnum == 3 && role == 3 && accounts[0]) {

        window.location.replace(`/DoctorProfile?account=${accounts[0]}`);
      }
      else if (personnum == role && personnum == 4 && role == 4 && accounts[0]) {
      ...
      <main>
        ...
        <div className="text-center py-2 px-5 mx-5">
          <span className=" mt-5  mx-auto text-center">
            <h2 className="mb-1 p-2 border-2 border-info border-bottom ">Dashboards</h2>
          </span>
          <div className="entity row forms d-flex justify-content-evenly">
            <Link
              ...
              <p className="">Super Admin manages and adds hospitals, and doctors to the network. </p>
            </Link>
            ...
            <Link
              ...
              <p className="">Each hospital has its own doctors that can be added to the network. </p>
            </Link>
            ...
            <Link
              ...
              onClick={() => onClick(3)}
              className="col-xl-5 icon-box card requests p-4 py-5 bg-opacity-50 ">
              <div className="icon"><Icon icon="healthicons:doctor-male" className="i" /></div>
              <h4 className="">Doctors</h4>
              <p className="">Doctors can request to add records to patient medical record, view their records and modify them. </p>
            </Link>
            ...
            <Link
              ...
              <p className="">Patient has all control on his medical record, can accept or reject doctor requests, or give permissions to access his record. </p>
            </Link>
          </div>
        </div>
      </main>
